    ...
    match shape {
        ...
        MbarrierBasicShape::ArriveNoComplete => {
                ...
                || !is_integer(ctx, op.get_result(0), 64, Signedness::Unsigned)
            {
                return verify_err!(
                    op.loc(),
                    "mbarrier no-complete arrival requires a u32 count and u64 opaque state"
                );
            }
        }
...
#[pliron_op(
    name = "nvvm.mbarrier_arrive_no_complete_shared",
    format,
    interfaces = [NOpdsInterface<2>, NResultsInterface<1>],
)]
pub struct MbarrierArriveNoCompleteSharedOp;
impl MbarrierArriveNoCompleteSharedOp {
    pub fn new(op: Ptr<Operation>) -> Self {
        Self { op }
    }
...
impl Verify for MbarrierArriveNoCompleteSharedOp {
    fn verify(&self, ctx: &Context) -> Result<(), Error> {
        verify_mbarrier_basic(
            ctx,
            self.get_operation(),
            "nvvm.mbarrier_arrive_no_complete_shared",
            MbarrierBasicShape::ArriveNoComplete,
        )
    }
}
...
pub(super) fn register(ctx: &mut Context) {
    ...
    MbarrierArriveNoCompleteSharedOp::register(ctx);
