...
/// The source-level signature of one concrete CUDA kernel entry.
///
/// `#[cuda_module]` generates one of these constants for every non-generic
/// kernel. The same parsed function signature that drives typed launch
/// marshalling therefore remains available to registries and other generic
/// launch infrastructure without repeating the argument list.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct CudaKernelSignature {
    /// PTX entry-point name.
    pub entry: &'static str,
    /// Ordered source arguments and their launch-carrier representation.
    pub arguments: &'static [CudaKernelArgument],
}

/// One source argument in a [`CudaKernelSignature`].
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct CudaKernelArgument {
    /// Argument name in the Rust kernel declaration.
    pub name: &'static str,
    /// How the source argument is marshalled into the CUDA launch packet.
    pub kind: CudaKernelArgumentKind,
}

/// Launch-carrier representation selected by `#[cuda_module]`.
///
/// One source argument can occupy multiple launch-packet slots. In particular,
/// a device slice carries a pointer and length, plus an optional row width.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
#[non_exhaustive]
pub enum CudaKernelArgumentKind {
    /// A by-value scalar or aggregate.
    Scalar(CudaKernelScalarKind),
    /// A Rust raw pointer, passed as one device-address parameter.
    DevicePointer,
    /// A `#[grid_constant]` aggregate passed by value.
    GridConstant { size: usize, alignment: usize },
    /// A slice-like device buffer. The packet carries pointer and length, and
    /// optionally a runtime row width.
    DeviceSlice { writable: bool, row_width: bool },
}

/// Scalar identity when it is explicit in the Rust kernel declaration.
///
/// Aliases, user aggregates, and generic scalar types use `Opaque`; its
/// layout still describes the bytes passed by value.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
#[non_exhaustive]
pub enum CudaKernelScalarKind {
    Bool,
    U8,
    U16,
    U32,
