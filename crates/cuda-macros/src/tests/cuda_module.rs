...
fn grid_constant_uses_one_by_value_host_argument() {
    ...
    assert!(
        expanded.contains(
            ...
            "kind:::cuda_host::CudaKernelParameterKind::GridConstant{size:::core::mem::size_of::<TensorMap>(),alignment:::core::mem::align_of::<TensorMap>()"
        ) && expanded.contains(
            "name:\"out\",kind:::cuda_host::CudaKernelParameterKind::DevicePointer"
        ),
        "{expanded}"
    );
}
...
#[test]
fn concrete_kernel_signature_is_generated_from_launcher_parameters() {
    let module: ItemMod = parse_quote! {
        mod kernels {
            #[kernel]
    ...
    assert!(
        expanded.contains("pubconstCLASSIFY_CUDA_SIGNATURE:::cuda_host::CudaKernelSignature")
            && expanded.contains(
                "name:\"flag\",kind:::cuda_host::CudaKernelParameterKind::Scalar(::cuda_host::CudaKernelScalarKind::Bool)"
            )
            && expanded.contains(
                "name:\"count\",kind:::cuda_host::CudaKernelParameterKind::Scalar(::cuda_host::CudaKernelScalarKind::U32)"
            )
            && expanded.contains(
                "name:\"data\",kind:::cuda_host::CudaKernelParameterKind::DeviceSlice{writable:false,row_width:false"
            )
            && expanded.contains(
                "name:\"out\",kind:::cuda_host::CudaKernelParameterKind::DevicePointer"
            ),
